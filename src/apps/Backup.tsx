...
export function BackupApp(){
  ...
  const start=()=>{
    ...
    let v=0;
    const id=setInterval(()=>{ v+= Math.random()*18; if(v>=100){ v=100; clearInterval(id); setStep(2); } setProgress(Math.floor(v)); }, 300);
  };
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
      ...
      <Frame variant="well" style={{ padding:8, background:"#fff" }}>
        ...
        {step===2 && <div style={{ fontSize:11, color:"#000080" }}>✓ Backup complete. {Math.floor(progress*1.2)} files, 12.4 MB written.<br/>Verify: OK. Log: C:\BACKUP.LOG</div>}
      </Frame>
      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
        <Checkbox checked label="Verify after backup" value="verify" />
        <Checkbox checked={false} label="Compress" value="compress" />
      </div>
